...
namespace AWindowsService.DeviceRepository
{
    public class Zkclient
    {
        ...
        public bool GetUserLog(DeviceList device)
        {
            try
            {
                ...
                if (_isConnected == true)
                {
                    ...
                    if (cZKEM.ReadGeneralLogData(_iMachineNumber))//read all the attendance records to the memory
                    {
                        ...
                        var datas = cZKEM.SSR_GetGeneralLogData(_iMachineNumber, out idwEnrollNumber, out idwVerifyMode, out idwInOutMode, out idwYear, out idwMonth, out idwDay, out idwHour, out idwMinute, out dwSecond, ref dwWorkCode);
                        ...
                        if (device.IsAttendance == "1")
                        {
                            if (PostLog.log != null)
                            {
                                ...
                                using (var client = new HttpClient())
                                {
                                    ...
                                    StaticVariables.Log($"Request from WEB API..!" + JsonRequest.ToString());
                                    ...
                                    StaticVariables.Log($"Response from WEB API..!" + responseStr.ToString());
                                    Console.WriteLine($"Response from WEB API..!"+ JsonResponse.ToString());
                                }
            ...
            {
                StaticVariables.Log(string.Format(exp.Message));
                _status = false;
            }
            finally
            {
